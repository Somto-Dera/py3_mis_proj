...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        if self.prizeMoney < VOWEL_COST:
            new_list = [letter for letter in self.SORTED_FREQUENCIES if letter not in VOWELS]
            
        else:
            new_list = [letter for letter in self.SORTED_FREQUENCIES if letter not in self.guessed]
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        check = self.getPossibleLetters(guessed)
        if len(check) == 0:
            return "pass"
        
        if self.smartCoinFlip() == True:
            res = [letter for letter in self.SORTED_FREQUENCIES if letter in check][-1]
            #"".join(res)
            return res
            #return self.SORTED_FREQUENCIES[len(self.SORTED_FREQUENCIES)-1]
        else:
